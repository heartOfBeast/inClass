 ...
 * Dynamic Web project : model2(mvc)를 구현 시 httpServlet 상속클래스에서 Http Request 처리 --> web.xml
 * spring에서 매핑 작업없이 Annotation만으로 서블릿 구현
 * 
 ...
 * 2. 이에 매핑되는 요청일 때만 해당 메소드가 처리 : Handler Adapter
 * 3. 해당 메소드는 view name 반환
 * 4. View Resolver ( "resources/templates/" + {view name} + ".html" ) 사용자가 요청 한 것에 대한 응답 view를 랜더링하는 역할
 * 5. 해당 뷰 템플릿에서 전달 된 model 속 객체 사용가능 
 * */	
